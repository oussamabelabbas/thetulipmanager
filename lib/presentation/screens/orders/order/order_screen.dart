            ...
            child: BlocBuilder<OrderWatcherBloc, OrderWatcherState>(
              builder: (context, state) {
                return Theme(
                  data: state.maybeMap(
                    fetchsucceded: (state) =>
                        state.order.orderState == OrderState.archived ? TheTulipThemes.dark : Theme.of(context),
                    ...
                    appBar: state.maybeMap(
                      ...
                      fetchsucceded: (state) => AppBar(
                        leading:
                            IconButton(onPressed: context.router.pop, icon: const Icon(IconlyBroken.arrow___left_2)),
                        actions: [
                          ActionButton(orderId: orderId, isDone: state.order.orderState == OrderState.delivered)
                        ],
                      ),
                    ),
                          ...
                          return Stack(
                            children: [
                              SingleChildScrollView(
                                ...
                                child: AbsorbPointer(
                                  absorbing: order.orderState == OrderState.archived,
                                  child: Column(
                                      ...
                                      if (order.orderState == OrderState.delivered)
                                        ElevatedButton.icon(
                                          onPressed: () => context
                                              ...
                                              .add(ToggleOrderState(orderId: orderId, orderState: OrderState.archived)),
                              ...
                              if (order.orderState == OrderState.archived)
                                Center(
                                  child: Card(
                                    child: GestureDetector(
                                      onDoubleTap: () => context
                                          .read<OrdersActorBloc>()
                                          .add(ToggleOrderState(orderId: orderId, orderState: OrderState.delivered)),
                                      child: Padding(
                                        padding: const EdgeInsets.all(16.0),
